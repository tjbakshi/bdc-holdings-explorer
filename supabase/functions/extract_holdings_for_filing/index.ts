...
type ParserType = 'ARCC' | 'GBDC' | 'BXSL' | 'OBDC' | 'CGBD' | 'GENERIC';
...
function determineParserType(ticker: string | null, bdcName: string): ParserType {
  ...
  if (tickerUpper === 'CGBD' || nameUpper.includes('CARLYLE')) {
    return 'CGBD';
  }

  // Default to BXSL parser for unknown BDCs (better than GENERIC)
  return 'BXSL';
}
...
/**
 * ============================================================================
 * CGBD STREAMING PARSER - Optimized for Carlyle Secured Lending filings
 * ============================================================================
 * 
 * Based on BXSL slicing method - processes tables one at a time and inserts
 * holdings in batches to avoid memory limits.
 * 
 * CGBD-specific adjustments:
 * - Company column: 'Investments' or 'Portfolio Company'
 * - Industry column: explicit 'Industry' header
 * - Reference Rate: separate 'Reference Rate' column
 * - Spread: separate 'Spread' column  
 * - Interest Rate: 'Interest Rate' column
 * - Maturity: 'Maturity Date' column
 * - Principal: 'Principal' or 'Par' column
 * - Cost: 'Amortized Cost' column
 * - Fair Value: 'Fair Value' column
 * 
 * Special: Combines Reference Rate + Spread into reference_rate field (e.g., "SOFR + 5.50%")
 */
async function parseCGBDTableAndInsert(params: {
  html: string;
  filingId: string;
  supabaseClient: any;
  debugMode?: boolean;
  ...
  console.log(`\n🔷 CGBD STREAMING PARSER: Starting (input size: ${(html.length / 1024 / 1024).toFixed(2)} MB)`);
  console.log(`🔧 CGBD: Using Slicing Method for ${(html.length / 1024 / 1024).toFixed(0)}MB file.`);
  ...
  const defaultPeriodDateISO = extractPeriodDateISO(soiHeaderSection);
  if (defaultPeriodDateISO) {
    console.log(`🔧 CGBD: Default Period Date (SOI header) = ${defaultPeriodDateISO}`);
  } else {
    console.log(`⚠️ CGBD: Could not detect default period date from SOI header.`);
  }

  console.log(`🔍 STEP 3: Extracting post-SOI section...`);
  const MAX_SEARCH = 10_000_000;
  const afterSoi = html.slice(soiStart, Math.min(soiStart + MAX_SEARCH, html.length));
  console.log(`   Extracted ${(afterSoi.length / 1024).toFixed(0)} KB after SOI header`);
  ...
  // Clear existing holdings for this filing
  const { error: deleteError } = await supabaseClient
    .from("holdings")
    .delete()
    .eq("filing_id", filingId);
  ...
  let savedColIndices = {
    ...
    investmentType: -1,
    industry: -1,
    referenceRate: -1,  // CGBD: separate 'Reference Rate' column
    spread: -1,         // CGBD: separate 'Spread' column
    interestRate: -1,   // CGBD: 'Interest Rate' column
    maturity: -1,
    par: -1,
    cost: -1,
    fairValue: -1,
  ...
  while (searchPos < afterSoi.length && tableCount < MAX_TABLES && insertedCount < MAX_HOLDINGS) {
    ...
    // CGBD-specific column detection
    let colIndices = { ...savedColIndices };
    if (!hasFoundFirstTable) {
      for (let i = 0; i < Math.min(5, rows.length); i++) {
        const cells = Array.from(rows[i].querySelectorAll("th, td")) as Element[];
        ...
        for (const cell of cells) {
          ...
          // CGBD: 'Investments' or 'Portfolio Company' for company column
          if ((text.includes("investments") || text.includes("portfolio company") || text.includes("borrower")) && !text.includes("type") && colIndices.company === 0) {
            colIndices.company = position;
          }
          // CGBD: explicit 'Industry' column
          else if (text.includes("industry") && colIndices.industry === -1) {
            colIndices.industry = position;
          }
          // CGBD: 'Reference Rate' column
          else if (text.includes("reference rate") && colIndices.referenceRate === -1) {
            colIndices.referenceRate = position;
          }
          // CGBD: 'Spread' column
          else if (text.includes("spread") && colIndices.spread === -1) {
            colIndices.spread = position;
          }
          // CGBD: 'Interest Rate' column
          else if (text.includes("interest rate") && !text.includes("reference") && colIndices.interestRate === -1) {
            colIndices.interestRate = position;
          }
          // Maturity Date
          else if (text.includes("maturity") && colIndices.maturity === -1) {
    ...
    for (let i = dataStartRow; i < rows.length && insertedCount + pending.length < MAX_HOLDINGS; i++) {
      ...
      if (colIndices.referenceRate >= 0) {
        const refCell = getCellAtPos(colIndices.referenceRate);
      ...
      if (colIndices.spread >= 0) {
        const spreadCell = getCellAtPos(colIndices.spread);
        const spreadText = spreadCell?.textContent?.trim();
        if (spreadText && spreadText.length > 0 && !/^[-—\s]*$/.test(spreadText) && spreadText !== 'N/A') {
          // Format as "SOFR + 5.50%" style
          refRateParts.push(`+ ${spreadText}`);
        }
      }
      ...
      // Also capture standalone Interest Rate column
      if (colIndices.interestRate >= 0) {
        const rateCell = getCellAtPos(colIndices.interestRate);
        const rateText = rateCell?.textContent?.trim();
        if (rateText && rateText.length > 0 && !/^[-—\s]*$/.test(rateText) && rateText !== 'N/A') {
          interestRate = rateText;
        }
      }
  ...
  console.log(`\n🔷 CGBD STREAMING: Completed - inserted ${insertedCount} holdings from ${holdingsTableCount} tables (${totalProcessedRows} processed, ${totalRowsSkipped} skipped)`);

  if (insertedCount === 0) {
    console.log(`⚠️ CGBD: No holdings inserted; check table detection/column mapping logs above.`);
  }
      ...
      for (let docIdx = 0; docIdx < maxDocsToTry; docIdx++) {
        ...
        try {
          ...
          if (html.length > 5_000_000) {
            ...
            // For CGBD, use streaming parser (same approach as BXSL)
            if (parserType === 'CGBD') {
              console.log(`   🔀 CGBD detected - using STREAMING insert parser for large file`);

              const { insertedCount, scaleResult: cgbdScale } = await parseCGBDTableAndInsert({
                html,
                filingId,
                supabaseClient,
                debugMode,
              ...
              if (insertedCount > 0) {
                ...
                return new Response(
                  JSON.stringify({
                    filingId,
                    holdingsInserted: insertedCount,
                    valueScale: cgbdScale.detected,
                    scaleConfidence: cgbdScale.confidence,
                    method: 'cgbd-streaming',
                  }),
                  { headers: { ...corsHeaders, "Content-Type": "application/json" } }
                );
              }

              console.log(`   ⚠️ CGBD streaming parser found no holdings, trying next document...`);
              continue;
            }
